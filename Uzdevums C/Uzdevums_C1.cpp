...
apstrādā masīvu mas, kura elementi ir cipari skaitā gar.
Atrod visbiežāko ciparu, ja tādi cipari ir vairāki, atgriež lielāko.
***************************************************/
int visbiezakais (int *mas, int gar){
int reize;
int maks=0;
int elem;
    for(int i=0;i<gar;++i){
        ...
        if (reize>maks){
           ...
           if (elem>mas[i]) elem = elem;
           else elem = mas[i];                  ///ja vairāki cipari skaitļa pierakstā parādās vairākas reizes, atgriež lielāko.
        }
    }
return elem;
}
...
cin>>n;                                                                ///lietotāja ievadītais n skaitlis.
if(n<1) cout<<"Nekorekta vērtība! Skaitlim jābut naturālam!"<<endl;
}while(n<1);
n1=n;
n2=n;
while(n1>0){           ///atrod mainīgā gar vērtību.
    gar++;
    n1/=10;
}
...
for(int i=0;i<gar;++i){     ///ieraksta skaitļa n ciparus dinamiskajā masīvā mas.
    mas[i] = n2%10;
    n2/=10;
}
